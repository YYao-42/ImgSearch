...
from src.datasets.datahelpers import default_loader, imresize, cid2filename
    ...
    def create_epoch_tuples(self, net):
        ...
        with torch.no_grad():
            ...
            for i, input in enumerate(loader):
                if net.dquantiz is not None:
                    _ , _, _, qvecs_temp = net(input.cuda())
                    qvecs[:, i] = qvecs_temp.data.squeeze()
                else:                   
            ...
            for i, input in enumerate(loader):
                if net.dquantiz is not None:
                    _, _, _, poolvecs_temp = net(input.cuda())
                    poolvecs[:, i] = poolvecs_temp.data.squeeze()
                else:
